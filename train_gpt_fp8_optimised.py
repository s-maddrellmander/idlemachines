"""
train_gpt_fp8_optimized.py
FP8 Training Script - Optimized for Maximum Throughput
=======================================================

Optimizations over original:
1. Removed per-step torch.cuda.synchronize() (only sync for timing every N steps)
2. Added torch.set_float32_matmul_precision('high') for TF32
3. More aggressive torch.compile settings
4. Gradient norm computed only on log steps
5. Async-friendly data loading hints
6. Optional CUDA graphs for small models (experimental)

Usage:
    torchrun --nproc_per_node=8 train_gpt_fp8_optimized.py --model-size 125M --steps 1500
"""
...
# Enable TF32 for matmuls (significant speedup on Ampere+)
...
# -----------------------------------------------------------------------------
# Data Loading (with prefetch hint)
# -----------------------------------------------------------------------------
...
class FileDataLoader:
    ...
    def __init__(self, filename_pattern, B, T, rank=0, world_size=1):
        ...
        # Pre-allocate tensors for faster data loading
        self._x_buffer = torch.empty((B, T), dtype=torch.long, device='cuda', pin_memory=False)
        self._y_buffer = torch.empty((B, T), dtype=torch.long, device='cuda', pin_memory=False)
    ...
    def next_batch(self):
        ...
        x = buf[:-1].view(self.B, self.T).cuda(non_blocking=True)
        y = buf[1:].view(self.B, self.T).cuda(non_blocking=True)
...
# -----------------------------------------------------------------------------
# Gradient Norm (only compute when needed)
# -----------------------------------------------------------------------------
...
def train(args):
    ...
    print0("="*70)
    print0("FP8 TRAINING - OPTIMIZED FOR THROUGHPUT")
    print0("="*70)
    ...
    # Compile model - more aggressive settings
    print0(f"\n{'Compilation':-^50}")
    print0("Compiling model with torch.compile()...")
    model = torch.compile(model, fullgraph=False)  # More aggressive than default
    ...
    # For throughput measurement - only sync every N steps
    timing_sync_interval = args.log_every
    last_sync_time = time.time()
    last_sync_step = start_step
    ...
    for step in range(start_step + 1, args.steps + 1):
        ...
        # Only compute gradient norm on logging steps (expensive operation)
        grad_norm = 0.0
        ...
        # Only sync for accurate timing on log steps
        if step % timing_sync_interval == 0 or step == 1:
            torch.cuda.synchronize()
            current_time = time.time()
            
            # Calculate throughput over the interval
            steps_since_sync = step - last_sync_step
            time_since_sync = current_time - last_sync_time
            tokens_since_sync = steps_since_sync * tokens_per_step
            tokens_per_sec = tokens_since_sync / time_since_sync if time_since_sync > 0 else 0
            
            step_time = time_since_sync / steps_since_sync if steps_since_sync > 0 else 0
            train_time_total += time_since_sync
            
            last_sync_time = current_time
            last_sync_step = step
        else:
            step_time = 0  # Not measured this step
            tokens_per_sec = 0
        ...
        if args.val_every > 0 and step % args.val_every == 0:
            torch.cuda.synchronize()  # Sync before validation
        ...
        # Log to CSV
        if is_master and csv_writer and (step % args.log_every == 0 or step == 1):
            csv_writer.writerow([
                step,
                f"{loss_val:.4f}",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='FP8 Training - Optimized')
